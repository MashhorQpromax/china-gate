...
export default function LCPage() {
  const [lcs, setLcs] = useState<ApiLC[]>([]);
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401 || res.status === 403) {
          setError('Please log in to view letters of credit');
          return;
        }
      ...
      if (json.success) {
        let data = json.data || [];
        if (statusFilter !== 'all') {
          data = data.filter((lc: ApiLC) => lc.status === statusFilter);
        }
        if (typeFilter !== 'all') {
          data = data.filter((lc: ApiLC) => lc.lc_type === typeFilter);
        }
        setLcs(data);
  ...
  // Stats
  const activeCount = lcs.filter(lc => !['expired', 'cancelled'].includes(lc.status)).length;
  const totalValue = lcs.reduce((sum, lc) => sum + (lc.amount || 0), 0);
  const expiringSoon = lcs.filter(lc => {
    const days = getDaysUntilExpiry(lc.expiry_date);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total LCs', value: total, color: 'text-blue-400', icon: '📄' },
            ...
            { label: 'Expiring Soon', value: expiringSoon, color: 'text-yellow-400', icon: '⏰' },
          ].map((stat) => (
        ...
        {error && (
          <div className="bg-[#1a1f2b] border border-red-600 rounded-lg p-6 text-center">
            ...
            <button onClick={fetchLCs} className="px-4 py-2 bg-[#c41e3a] text-white rounded-lg hover:bg-red-700 text-sm">Retry</button>
          </div>
        )}
        ...
        {loading && (
          <div className="bg-[#1a1f2b] border border-gray-700 rounded-lg p-12 text-center">
            ...
            <p className="text-gray-400">Loading letters of credit...</p>
          </div>
        )}
          ...
          <div className="bg-[#1a1f2b] rounded-lg border border-gray-700 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {lcs.length === 0 && (
                    <tr><td colSpan={8} className="py-8 text-center text-gray-500">No letters of credit found</td></tr>
                  )}
                  {lcs.map((lc, idx) => {
                    const daysToExpiry = getDaysUntilExpiry(lc.expiry_date);
                    ...
                    return (
                      <tr key={lc.id} className={`border-b border-gray-700 hover:bg-[#252d3a] transition-colors ${idx % 2 === 0 ? 'bg-[#141820]' : ''}`}>
                        <td className="px-6 py-4 text-white font-semibold">
                          <a href={`/lc/${lc.id}`} className="text-[#d4a843] hover:text-yellow-300">{lc.reference_number}</a>
                        </td>
                        <td className="px-6 py-4 text-gray-300 text-sm">{lc.deals?.reference_number || '-'}</td>
                        <td className="px-6 py-4 text-gray-300 text-sm">{lc.issuing_bank || '-'}</td>
                        <td className="px-6 py-4 text-white font-semibold">{formatCurrency(lc.amount, lc.currency)}</td>
                        <td className="px-6 py-4 text-gray-300 text-sm capitalize">{(lc.lc_type || '').replace(/_/g, ' ')}</td>
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColors[lc.status] || 'bg-gray-500/20 text-gray-300'}`}>
                        ...
                        <td className="px-6 py-4">
                          <a href={`/lc/${lc.id}`} className="text-[#d4a843] hover:text-yellow-300 text-sm font-semibold">View →</a>
                        </td>
                      </tr>
                    );
                  })}
